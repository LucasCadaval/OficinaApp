...
//    }
    ...
    fun fetchVeiculos() {
        viewModelScope.launch {
            ...
            _error.value = null
            try {
                db.collection("users").document(userId).collection("veiculos").addSnapshotListener { value, _ ->
            val veiculos = value?.toObjects(Veiculo::class.java) ?: emptyList()
            _veiculos.value = veiculos
        }
            } catch (e: Exception) {
    ...
    fun addVeiculo(veiculo: Veiculo, onComplete: () -> Unit) {
        db.collection("users").document(userId).collection("veiculos").add(veiculo).addOnSuccessListener { onComplete() }
    }
